...
public class SettingsActivity extends AppCompatPreferenceActivity implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
        if (key.equals(BundleKeys.KEY_PREF_STORE_HISTORY)) {
            boolean storeHistory = sharedPreferences.getBoolean(key, true);

            if (!storeHistory) {
                try {
                    DeleteBuilder<City, Long> deleteBuilder = getHelper().getCityDao().deleteBuilder();
                    deleteBuilder.delete();
                } catch (SQLException e) {
                    Toast.makeText(this, R.string.error_database_query, Toast.LENGTH_LONG).show();
                    e.printStackTrace();
                }
            }
        }
